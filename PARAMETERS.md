...
legacy pixel expansion. When the optional GigaTIME branch is enabled, its
default storage is all channels in float32.
...
| `cell_detection_mode` | `stardist` | Scientific cell-identification contract. The default runs StarDist only. `consensus` explicitly adds HoVer-Net MoNuSAC and CellViT++ and requires GPU execution. Hardware never changes this mode automatically. |
...
| Parameter | Default | Meaning |
|---|---|---|
| `gigatime_enable` | `false` | Optional crop-image GigaTIME virtual mIF branch after StarDist. Set to `true` only when virtual-marker inference is required; otherwise the model path/token, inference, marker quantification and marker-derived KODAMA branch are not required. |
...
| `gigatime_kodama_enable` | `false` | Optionally run an independent PCA/KODAMA analysis on all matched nuclei and cytoplasm marker means. Requires `gigatime_enable=true` with integrated quantification. |
...
| Parameter | Default | Meaning |
|---|---|---|
| `marker_quantification_enable` | `false` | Optionally quantify a configured GigaTIME marker stack over nuclei and cytoplasm label masks. The normal no-GigaTIME route leaves this disabled. |
