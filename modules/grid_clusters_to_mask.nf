process GRID_CLUSTERS_TO_MASK {
    cache 'deep'
    ext code_fingerprint: { ProcessCode.fingerprint(projectDir, 'grid_clusters_to_mask', params) },
        source_fingerprint: { ProcessCode.directoryFingerprint([grid_objects_csv, grid_metadata_json, cluster_csv, preview_image_tif]) }
    ...
    input:
    tuple val(sample_key), val(sample_id), val(cluster_variant), path(grid_objects_csv), path(grid_metadata_json), path(cluster_csv), path(preview_image_tif)
    ...
    script:
    def scriptPath = "${projectDir}/${params.uni2_grid_cluster_mask_script}"
    def codeDigest = java.security.MessageDigest.getInstance('SHA-256')
    [scriptPath, "${projectDir}/bin/labels_to_cluster_mask.py", "${projectDir}/bin/tiff_preview.py", "${projectDir}/bin/ome_tiff_metadata.py"].each {
    ...
    echo "[INFO] UNI2 grid cluster-mask code fingerprint: ${codeFingerprint}"

    python "${scriptPath}" \
      --grid-objects "${grid_objects_csv}" \
      --grid-metadata "${grid_metadata_json}" \
      --map "${cluster_csv}" \
    ...
    printf '{"observation_type":"spatial_grid","abstained_observations":0}\n' > "${sample_id}_${cluster_variant}_cluster_mask_summary.json"
    """
}
