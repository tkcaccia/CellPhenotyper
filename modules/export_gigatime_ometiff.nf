process EXPORT_GIGATIME_OMETIFF {
    cache 'deep'
    ext code_fingerprint: { ProcessCode.fingerprint(projectDir, 'export_gigatime_ometiff', params) },
        source_fingerprint: { ProcessCode.directoryFingerprint([gigatime_dir]) }
    tag "${sample_id}"
    ...
    cpus { Math.max(1, Math.min(params._executor_max_cpus as int, params.gigatime_export_cpus as int)) }
    memory { "${Math.max(2, Math.min(params._executor_max_memory_gb as int, params.gigatime_export_memory_gb as int))} GB" }
